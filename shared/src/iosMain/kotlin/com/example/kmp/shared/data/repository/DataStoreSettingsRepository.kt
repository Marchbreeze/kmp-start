...
class DataStoreSettingsRepository(
    ...
    override suspend fun updateDarkTheme(isDarkTheme: Boolean) {
        dataStore.edit { it[PreferencesKeys.IS_DARK_THEME] = isDarkTheme }
    }

    override suspend fun updateUseSystemTheme(useSystemTheme: Boolean) {
        dataStore.edit { it[PreferencesKeys.USE_SYSTEM_THEME] = useSystemTheme }
    }

    override suspend fun updateCharactersPerPage(count: Int) {
        dataStore.edit { it[PreferencesKeys.CHARACTERS_PER_PAGE] = count }
    }

    override suspend fun updateLastViewedCharacterId(characterId: Int) {
        dataStore.edit { it[PreferencesKeys.LAST_VIEWED_CHARACTER_ID] = characterId }
    }
}
